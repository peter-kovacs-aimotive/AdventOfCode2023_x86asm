...
endPointCalculation:
	...
	; init number of each original card to 1
	lea di, numberOfCards
	mov cx, word[numLines]
	mov ax, 1
	rep stosw
	
	; now calculate copies
	lea si, numberOfWins
	mov cx, 0
calculateCopiesOfNextCard:	
	push cx
	lodsw
	cmp ax, 0
	jz doNotIncrement
...
incrementNumberOfCards:
	inc word[es:di]
	add di, 2
	loop incrementNumberOfCards
doNotIncrement:
	pop cx
	
	inc cx
	cmp cx, word[numLines]
	jnz calculateCopiesOfNextCard
	
	; sum copies
	xor ebp, ebp
	mov cx, word[numLines]
	lea si, numberOfCards
sumCopies:
	lodsw
	add bp, ax
	loop sumCopies
	
	;mov ebp, dword[totalNumberOfCards]
	call printResult
...
numberOfWins	resw 256
numberOfCards	resw 256

bytesRead	dw 0

totalNumberOfCards	dd 0
